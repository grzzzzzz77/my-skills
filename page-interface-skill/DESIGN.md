...
- 右侧字段卡片采用渐进披露：默认展示字段名、sourceType、confidence、前端字段、API 字段、接口/normalize、前端逻辑；完整 traceChain、证据和字段风险放入可展开区域。
- 右侧字段卡片不要把编号链路、证据 chip、风险说明全部默认展开。默认视图要先给结论和口径，详细证明按需展开。
