...
# -*- coding: utf-8 -*-
"""
build_docx.py —— 把求职分析与发展规划报告的 Markdown 转成 Word(.docx)。
...
用法:
    ...
    # 默认无 logo（中性排版，不挂任何品牌）；仅当显式传入第三个参数时才在页眉右上角显示 logo
...
页眉：默认无（中性排版，不挂任何品牌）；仅在显式传入 logo 时右上角显示。页脚：居中页码"第 X 页"。
"""
...
def main():
    ...
    # 默认无 logo（中性排版）；仅当显式传入第三个参数时才挂 logo
